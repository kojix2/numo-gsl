[{:func_name=>"gsl_multifit_linear",
 ...
 {:func_name=>"gsl_multifit_linear_Lsobolev",
  ...
  :args=>
   [["const size_t", "p"],
    ...
    ["gsl_multifit_linear_workspace *", "work"]],
  :desc=>
   "This function computes the regularization matrix L corresponding to the\n" +
   "weighted Sobolov norm\n" +
   "||L c||^2 = \\sum_k \\alpha_k^2 ||L_k c||^2 where L_k approximates\n" +
   "the derivative operator of order k. This regularization norm can be useful\n" +
   "in applications where it is necessary to smooth several derivatives of the solution.\n" +
   "p is the number of model parameters, kmax is the highest derivative\n" +
   "to include in the summation above, and alpha is the vector of weights of\n" +
   "size kmax + 1, where alpha[k] = \\alpha_k is the weight\n" +
 ...
 {:func_name=>"gsl_multifit_linear_lcorner2",
  ...
  :desc=>
   ...
   "in idx. If the input arrays contain colinear points,\n" +
   "this function could fail and return GSL_EINVAL."},
 {:func_name=>"gsl_multifit_linear_lcurvature",
  :func_type=>"int",
  :args=>
   [["const gsl_vector *", "y"],
    ["const gsl_vector *", "reg_param"],
  ...
  :desc=>
   ...
   "(\\log{\\hat{\\rho}(\\lambda)}, \\log{\\hat{\\eta}(\\lambda)}), where\n" +
   "\\hat{\\rho}(\\lambda) = \\log{||y - X c_{\\lambda}||} and\n" +
   "\\hat{\\eta}(\\lambda) = \\log{|| L c_{\\lambda} ||}.\n" +
   "This function uses the right hand side vector y,\n" +
   "the vector of regularization parameters, reg_param,\n" +
   "vector of residual norms, rho, and vector of solution norms, eta.\n" +
   "The arrays reg_param, rho, and eta can be computed by\n" +
   "gsl_multifit_linear_lcurve.  The curvature is defined as\n" +
   ".. math:: \\kappa(\\lambda) = \\frac{\\hat{\\rho}' \\hat{\\eta}'' - \\hat{\\rho}'' \\hat{\\eta}'}{\\left( (\\hat{\\rho}')^2 + (\\hat{\\eta}')^2 \\right)^{\\frac{3}{2}}}\n" +
   "The curvature values are stored in kappa on output. The function\n" +
   "gsl_multifit_linear_svd must be called on the least squares matrix\n" +
   "X prior to calling this function."},
 {:func_name=>"gsl_multifit_linear_lcurvature_menger",
  :func_type=>"int",
  :args=>
   [["const gsl_vector *", "rho"],
    ["const gsl_vector *", "eta"],
  ...
  :desc=>
   ...
   "\\kappa_i = 1/R_i, where R_i is the radius of the unique\n" +
   "circle fitted to the points\n" +
   "(\\hat{\\rho}_{i-1}, \\hat{\\eta}_{i-1}), (\\hat{\\rho}_i, \\hat{\\eta}_i), (\\hat{\\rho}_{i+1}, \\hat{\\eta}_{i+1}).\n" +
   "The vector of residual norms || y - X c_{\\lambda} || is provided in rho,\n" +
   "the vector of solution norms || L c_{\\lambda} || is provided in eta.\n" +
   "These arrays can be calculated by calling gsl_multifit_linear_lcurve.\n" +
   "The Menger curvature output is stored in kappa. The Menger curvature is\n" +
   "an approximation to the curvature calculated by gsl_multifit_linear_lcurvature but\n" +
 ...
 {:func_name=>"gsl_multifit_linear_lcurve",
  ...
  :desc=>
   ...
   "the L-curve."},
 {:func_name=>"gsl_multifit_linear_lreg",
  :func_type=>"int",
  :args=>
   [["const double", "smin"],
    ["const double", "smax"],
  ...
  :desc=>
   ...
   "and stores them in the output vector reg_param of length k, where k is\n" +
   "the number of desired points on the L-curve. The regularization parameters are equally logarithmically\n" +
   "distributed between the provided values smin and smax, which typically correspond to\n" +
 ...
 {:func_name=>"gsl_multifit_linear_rcond",
  ...
  :desc=>
   ...
   "rcond = \\sigma_{min}/\\sigma_{max}.\n" +
   "The routine gsl_multifit_linear_svd must first be called to compute the SVD\n" +
   "of X."},
 ...
 {:func_name=>"gsl_multifit_linear_solve",
  ...
  :desc=>
   ...
   "The residual norm || \\tilde{y} - \\tilde{X} \\tilde{c} || = ||y - X c||_W\n" +
 ...
 {:func_name=>"gsl_multifit_nlinear_alloc",
  ...
  :desc=>
   ...
   "code of GSL_ENOMEM."},
 {:func_name=>"gsl_multifit_nlinear_avratio",
  :func_type=>"double",
  :args=>[["const gsl_multifit_nlinear_workspace *", "w"]],
  :desc=>
   "This function returns the current ratio |a| / |v| of the acceleration correction term to\n" +
 ...
 {:func_name=>"gsl_multifit_nlinear_init",
  ...
  :desc=>
   ...
   "x. See :ref:`sec_providing-function-minimized`\n" +
 ...
 {:func_name=>"gsl_multifit_nlinear_rcond",
  ...
  :desc=>
   ...
   "* For the QR solver, J is factored as J = Q R at each\n" +
 ...
 {:func_name=>"gsl_multifit_robust_est",
  ...
  :desc=>
   ...
   "c and their covariance matrix\n" +
   "cov to compute the fitted function value\n" +
   "y and its standard deviation y_err for the model y = x \\cdot c \n" +
   "at the point x."},
 ...
 {:func_name=>"gsl_multifit_robust_statistics",
  ...
  :desc=>
   ...
   "The function gsl_multifit_robust must be called first to perform the regression and\n" +
 ...
 {:func_name=>"gsl_multifit_robust_tune",
  ...
  :desc=>
   "This function sets the tuning constant t used to adjust the residuals at each\n" +
   "iteration to tune.  Decreasing the tuning constant increases the downweight\n" +
 ...
 {:func_name=>"gsl_multifit_robust_weights",
  ...
  :desc=>
   ...
   "r and previously specified weighting function. The output weights are given\n" +
   "by wts_i = w(r_i / (t \\sigma)), where the weighting functions w are\n" +
   "detailed in gsl_multifit_robust_alloc. \\sigma is an estimate of the\n" +
   "residual standard deviation based on the Median-Absolute-Deviation and t\n" +
